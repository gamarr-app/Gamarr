...
namespace NzbDrone.Core.Test.DecisionEngineTests
{
    ...
    public class MonitoredGameSpecificationFixture : CoreTest<MonitoredGameSpecification>
    {
        ...
        {
            _monitoredEpisodeSpecification.IsSatisfiedBy(_parseResultSingle, new ReleaseDecisionInformation()).Accepted.Should().BeTrue();
            _monitoredEpisodeSpecification.IsSatisfiedBy(_parseResultMulti, new ReleaseDecisionInformation()).Accepted.Should().BeTrue();
        }
        ...
        {
            _fakeSeries.Monitored = false;
            _monitoredEpisodeSpecification.IsSatisfiedBy(_parseResultMulti, new ReleaseDecisionInformation()).Accepted.Should().BeFalse();
        }
        ...
        {
            WithGameUnmonitored();
            _monitoredEpisodeSpecification.IsSatisfiedBy(_parseResultSingle, new ReleaseDecisionInformation()).Accepted.Should().BeFalse();
        }
        ...
        {
            _fakeSeries.Monitored = false;
            _monitoredEpisodeSpecification.IsSatisfiedBy(_parseResultSingle, new ReleaseDecisionInformation(false, new GameSearchCriteria { UserInvokedSearch = true })).Accepted.Should().BeTrue();
        }
    }
}
