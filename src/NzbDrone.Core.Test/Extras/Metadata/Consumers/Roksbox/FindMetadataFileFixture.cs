    ...
    public class FindMetadataFileFixture : CoreTest<RoksboxMetadata>
    {
        ...
        public void Setup()
        {
            _game = Builder<Game>.CreateNew()
                                     .With(s => s.Path = @"C:\Test\Games\The.Game.2011".AsOsAgnostic())
                                     .Build();
        }
        ...
        [TestCase(".jpg", MetadataType.GameImage)]
        ...
        {
            var path = Path.Combine(_game.Path, "the.game.here" + extension);
        ...
        {
            var path = Path.Combine(_game.Path, "the.game.2011-thumb.jpg");
        ...
        {
            var path = Path.Combine(_game.Path, new DirectoryInfo(_game.Path).Name + ".jpg");
