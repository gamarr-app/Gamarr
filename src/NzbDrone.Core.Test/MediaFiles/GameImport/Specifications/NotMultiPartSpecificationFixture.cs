...
namespace NzbDrone.Core.Test.MediaFiles.GameImport.Specifications
{
    ...
    public class NotMultiPartSpecificationFixture : CoreTest<NotMultiPartSpecification>
    {
        ...
        public void Setup()
        {
            ...
            {
                Path = @"C:\Test\Downloaded\somegame.avi".AsOsAgnostic()
            };
        }
        ...
        {
            _localGame.Path = paths.First().ToString().AsOsAgnostic();

            var filePaths = paths.Cast<string>().Select(x => x.AsOsAgnostic()).ToArray();

            Mocker.GetMock<IDiskProvider>()
                  .Setup(s => s.GetFiles(_localGame.Path.GetParentPath(), false))
                  .Returns(filePaths);
