...
namespace Gamarr.Api.V3.Queue
{
                               ...
                               IHandle<QueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public PagingResource<QueueResource> GetQueue([FromQuery] PagingRequestResource paging, bool includeUnknownGameItems = false, bool includeGame = false, [FromQuery] int[] gameIds = null, DownloadProtocol? protocol = null, [FromQuery] int[] languages = null, [FromQuery] int[] quality = null, [FromQuery] QueueStatus[] status = null)
        {
            ...
            return pagingSpec.ApplyToPage((spec) => GetQueue(spec, gameIds?.ToHashSet(), protocol, languages?.ToHashSet(), quality?.ToHashSet(), status?.ToHashSet(), includeUnknownGameItems), (q) => MapToResource(q, includeGame));
        }

        private PagingSpec<NzbDrone.Core.Queue.Queue> GetQueue(PagingSpec<NzbDrone.Core.Queue.Queue> pagingSpec, HashSet<int> gameIds, DownloadProtocol? protocol, HashSet<int> languages, HashSet<int> quality, HashSet<QueueStatus> status, bool includeUnknownGameItems)
        {
            var ascending = pagingSpec.SortDirection == SortDirection.Ascending;
            var orderByFunc = GetOrderByFunc(pagingSpec);

            var queue = _queueService.GetQueue();
            var filteredQueue = includeUnknownGameItems ? queue : queue.Where(q => q.Game != null);
        ...
        private Func<NzbDrone.Core.Queue.Queue, object> GetOrderByFunc(PagingSpec<NzbDrone.Core.Queue.Queue> pagingSpec)
        {
            switch (pagingSpec.SortKey)
            {
                ...
                case "games.sortTitle":
                    return q => q.Game?.GameMetadata.Value.SortTitle ?? q.Title;
                case "title":
                    return q => q.Title;
                case "year":
                    return q => q.Game?.Year ?? 0;
                case "languages":
        ...
        private void Remove(NzbDrone.Core.Queue.Queue pendingRelease, bool blocklist)
        {
            if (blocklist)
            {
                _blocklistService.Block(pendingRelease.RemoteGame, "Pending release manually blocklisted");
            }
        ...
        private QueueResource MapToResource(NzbDrone.Core.Queue.Queue queueItem, bool includeGame)
        {
            return queueItem.ToResource(includeGame);
        }
