...
namespace Gamarr.Api.V3.Queue
{
    ...
    {
        public static QueueResource ToResource(this NzbDrone.Core.Queue.Queue model, bool includeGame)
        {
            if (model == null)
            {
                return null;
            ...
            var customFormats = model.RemoteGame?.CustomFormats;
            var customFormatScore = model.Game?.QualityProfile?.CalculateCustomFormatScore(customFormats) ?? 0;
        ...
        public static List<QueueResource> ToResource(this IEnumerable<NzbDrone.Core.Queue.Queue> models, bool includeGame)
        {
            return models.Select((m) => ToResource(m, includeGame)).ToList();
        }
    }
}
