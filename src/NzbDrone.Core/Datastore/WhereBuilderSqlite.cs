    ...
    public class WhereBuilderSqlite : WhereBuilder
    {
        ...
        private void ParseEnumerableContains(MethodCallExpression body)
        {
            ...
            if (item.Type == typeof(int) && TryGetRightValue(list, out var value) && value is IEnumerable<int> ints)
            {
                ...
                if (items.Count == 0)
                {
                    // "IN ()" is a syntax error in SQLite. Contains over an empty set is
                    // false for every row, so degrade to a constant-false clause. Emitting
                    // no clause at all would silently widen the query to every row.
                    _sb.Append("(1 = 0)");
                    _gotConcreteValue = true;
                    return;
                }
