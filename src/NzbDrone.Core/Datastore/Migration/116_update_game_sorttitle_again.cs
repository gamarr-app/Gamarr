...
{
    [Migration(116)]
    public class update_game_sorttitle_again : NzbDroneMigrationBase
    {
        ...
        private void SetSortTitles(IDbConnection conn, IDbTransaction tran)
        {
            using (var getSeriesCmd = conn.CreateCommand())
            {
                ...
                getSeriesCmd.CommandText = @"SELECT ""Id"", ""Title"" FROM ""Games""";
                using (var seriesReader = getSeriesCmd.ExecuteReader())
                {
                    while (seriesReader.Read())
                    {
                        ...
                        using (var updateCmd = conn.CreateCommand())
                        {
                            ...
                            updateCmd.CommandText = "UPDATE \"Games\" SET \"SortTitle\" = ? WHERE \"Id\" = ?";
