...
namespace NzbDrone.Core.HealthCheck.Checks
{
    ...
    public class GameCollectionRootFolderCheck : HealthCheckBase
    {
        private readonly IGameCollectionService _collectionService;
        private readonly IDiskProvider _diskProvider;
        private readonly IRootFolderService _rootFolderService;

        public GameCollectionRootFolderCheck(IGameCollectionService collectionService, IDiskProvider diskProvider, IRootFolderService rootFolderService, ILocalizationService localizationService)
        ...
        public override HealthCheck Check()
        {
            ...
            var missingRootFolders = new Dictionary<string, List<GameCollection>>();
            ...
            foreach (var collection in collections)
            {
                ...
                {
                    missingRootFolders.Add(rootFolderPath, new List<GameCollection> { collection });
                }
            }
            ...
            if (missingRootFolders.Any())
            {
                if (missingRootFolders.Count == 1)
                {
                    ...
                    return new HealthCheck(GetType(),
                        ...
                        _localizationService.GetLocalizedString("GameCollectionRootFolderMissingRootHealthCheckMessage", new Dictionary<string, object>
                        {
                            { "rootFolderInfo", FormatRootFolder(missingRootFolder.Key, missingRootFolder.Value) }
                        }),
                        "#game-collection-missing-root-folder");
                }

                return new HealthCheck(GetType(),
                    HealthCheckResult.Error,
                    _localizationService.GetLocalizedString("GameCollectionFolderMultipleMissingRootsHealthCheckMessage", new Dictionary<string, object>
                    {
        ...
        private string FormatRootFolder(string rootFolderPath, List<GameCollection> collections)
        {
