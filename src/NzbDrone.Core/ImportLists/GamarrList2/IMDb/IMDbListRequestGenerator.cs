...
{
    public class IMDbListRequestGenerator : GamarrList2RequestGeneratorBase
    {
        public IMDbListSettings Settings { get; set; }

        protected override HttpRequest GetHttpRequest()
        {
            Logger.Info("IMDb List {0}: Importing games", Settings.ListId);

            // Use IMDb list Export for user lists to bypass GamarrAPI caching
