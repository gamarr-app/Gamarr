...
namespace NzbDrone.Core.Indexers.Newznab
{
    public class NewznabRequestGenerator : IIndexerRequestGenerator
    {
        ...
        private void AddGameIdPageableRequests(IndexerPageableRequestChain chain, int maxPages, IEnumerable<int> categories, SearchCriteriaBase searchCriteria)
        {
            ...
            {
                chain.AddTier();
                var queryTitles = TextSearchEngine == "raw" ? searchCriteria.SceneTitles : searchCriteria.SearchQueryTitles;

                foreach (var queryTitle in queryTitles)
                {
                    var searchTitle = queryTitle;

                    // Only include year if explicitly enabled (games rarely include year in name)
                    if (searchCriteria.Game.Year > 0 && Settings.IncludeYearInSearch)
                    {
