...
{
    public class AggregateLanguage : IAggregateLocalGame
    {
        public int Order => 1;
        ...
        {
            var languages = new List<Language> { localGame.Game?.GameMetadata.Value.OriginalLanguage ?? Language.Unknown };

            foreach (var augmentLanguage in _augmentLanguages)
            {
                var augmentedLanguage = augmentLanguage.AugmentLanguage(localGame, downloadClientItem);
