...
namespace NzbDrone.Update.UpdateEngine
{
    ...
    public class InstallUpdateService : IInstallUpdateService
    {
        ...
        public void Start(string installationFolder, int processId)
        {
            ...
            _logger.Info("Updating Gamarr from version {0} to version {1}", _detectExistingVersion.GetExistingVersion(installationFolder), BuildInfo.Version);

            Verify(installationFolder, processId);

            if (installationFolder.EndsWith(@"\bin\Gamarr") || installationFolder.EndsWith(@"/bin/Gamarr"))
            {
            ...
            _processProvider.FindProcessByName(ProcessProvider.GAMARR_CONSOLE_PROCESS_NAME);
            _processProvider.FindProcessByName(ProcessProvider.GAMARR_PROCESS_NAME);
            ...
            try
            {
                ...
                if (OsInfo.IsWindows)
                {
                    if (_processProvider.Exists(ProcessProvider.GAMARR_CONSOLE_PROCESS_NAME) || _processProvider.Exists(ProcessProvider.GAMARR_PROCESS_NAME))
                    {
                        _logger.Error("Gamarr was restarted prematurely by external process.");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        _diskProvider.SetFilePermissions(Path.Combine(installationFolder, "Gamarr"), "755", null);
            ...
            finally
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 10; i++)
                    {
                        ...
                        if (_processProvider.Exists(ProcessProvider.GAMARR_PROCESS_NAME))
                        {
                            _logger.Info("Gamarr was restarted by external process.");
                            break;
                        }
                    }
