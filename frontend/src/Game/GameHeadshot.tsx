import React from 'react';
import GameImage, { GameImageProps } from './GameImage';

const posterPlaceholder =
  'data:image/png;base64,';

interface GameHeadshotProps
  extends Omit<GameImageProps, 'coverType' | 'placeholder'> {
  size?: 250 | 500;
}

function GameHeadshot({ size = 250, ...otherProps }: GameHeadshotProps) {
  return (
    <GameImage
      {...otherProps}
      size={size}
      coverType="headshot"
      placeholder={posterPlaceholder}
    />
  );
}

export default GameHeadshot;
