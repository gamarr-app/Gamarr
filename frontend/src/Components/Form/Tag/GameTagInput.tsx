...
interface GameTag extends TagBase {
  id: number;
  name: string;
}

export interface GameTagInputProps<V> {
  name: string;
  value: V;
...
function createGameTagsSelector(tags: number[]) {
  return createSelector(createTagsSelector(), (tagList) => {
    ...
    return {
      tags: tags.reduce((acc: GameTag[], tag) => {
...
export default function GameTagInput<V>({
  name,
  value,
  onChange,
}: GameTagInputProps<V>) {
  ...
  const { tags, tagList, allTags } = useSelector(
    createGameTagsSelector(arrayValue)
  );
