...
interface Section {
  title: string;
  loading?: boolean;
  suggestions: GameSuggestion[] | AddNewGameSuggestion[];
}
...
function createUnoptimizedSelector() {
  return createSelector(
    ...
    createTagsSelector(),
    (allGames, allTags) => {
      return allGames.map((game): SuggestedGame => {
        const {
          title,
          year,
          titleSlug,
...
function GameSearchInput() {
  ...
  const renderSuggestion = useCallback(
    (
      item: AddNewGameSuggestion | GameSuggestion,
      { query }: { query: string }
    ) => {
      if ('type' in item) {
        return (
  ...
  const theme = {
    ...
    suggestionsContainer: styles.gameContainer,
