...
export const setGameFilesTableOption = createAction(
  SET_GAME_FILES_TABLE_OPTION
);
...
export const actionHandlers = handleThunks({
  ...
  [DELETE_GAME_FILE]: function (
    getState: () => AppState,
    payload: DeletePayload,
    dispatch: Dispatch
  ) {
    const { id: gameFileId, gameEntity = gameEntities.GAMES } = payload;
  ...
  [DELETE_GAME_FILES]: function (
    getState: () => AppState,
    payload: DeleteFilesPayload,
    dispatch: Dispatch
  ) {
    const { gameFileIds } = payload;
  ...
  [UPDATE_GAME_FILES]: function (
    ...
    promise.done((data: GameFile[]) => {
      dispatch(
        batchActions([
          ...files.map((file) => {
            const id = file.id;
            const props: Partial<GameFile> = {};
...
export const reducers = createHandleActions(
  {
    ...
    [CLEAR_GAME_FILES]: (state: typeof defaultState) => {
