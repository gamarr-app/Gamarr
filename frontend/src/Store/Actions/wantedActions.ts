...
interface Filter {
  key: string;
  label: () => string;
  filters: { key: string; value: boolean; type: string }[];
}

interface WantedSubState {
  isFetching: boolean;
  isPopulated: boolean;
  pageSize: number;
  sortKey: string;
...
export interface WantedState {
  missing: WantedSubState;
  cutoffUnmet: WantedSubState;
}

export const defaultState: WantedState = {
  missing: {
    isFetching: false,
    isPopulated: false,
    pageSize: 20,
...
export const BATCH_TOGGLE_MISSING_GAMES =
  'wanted/missing/batchToggleMissingGames';
...
export const BATCH_TOGGLE_CUTOFF_UNMET_GAMES =
  'wanted/cutoffUnmet/batchToggleCutoffUnmetGames';
...
export const batchToggleCutoffUnmetGames = createThunk(
  BATCH_TOGGLE_CUTOFF_UNMET_GAMES
);
...
export const actionHandlers = handleThunks({
  ...
  [BATCH_TOGGLE_MISSING_GAMES]: createBatchToggleGameMonitoredHandler(
    'wanted.missing',
    fetchMissing
  ),
  ...
  [BATCH_TOGGLE_CUTOFF_UNMET_GAMES]: createBatchToggleGameMonitoredHandler(
    'wanted.cutoffUnmet',
    fetchCutoffUnmet
  ),
});
