...
export const defaultState = {
  ...
  sortPredicates: {
    ...
    studio: function (item: Game): string {
      const studio = item.studio;

      return studio ? studio.toLowerCase() : '';
    },

    collection: function (item: Game): string | undefined {
      const { collection = {} as Game['collection'] } = item;

      return collection?.title;
    },

    originalLanguage: function (item: Game): string | undefined {
      const { originalLanguage = {} as Game['originalLanguage'] } = item;

      return originalLanguage?.name;
    },

    releaseGroups: function (item: Game): string[] | undefined {
    ...
    igdbRating: function ({ ratings = {} }: Game): number {
      return ratings.igdb ? ratings.igdb.value : 0;
    },

    metacriticRating: function ({ ratings = {} }: Game): number {
  ...
  filterBuilderProps: [
    ...
    },
    {
      ...
      optionsSelector: function (items: Game[]): OptionItem[] {
        const collectionList = items.reduce((acc: OptionItem[], game) => {
    ...
    },
    {
      ...
      optionsSelector: function (items: Game[]): OptionItem[] {
        const groupList = items.reduce((acc: OptionItem[], game) => {
    ...
    },
    {
      ...
      type: filterBuilderTypes.ARRAY,
      optionsSelector: function (items: Game[]): OptionItem[] {
        const genreList = items.reduce((acc: OptionItem[], { genres = [] }) => {
    ...
    },
    {
      ...
      type: filterBuilderTypes.ARRAY,
      optionsSelector: function (items: Game[]): OptionItem[] {
        const keywordList = items.reduce(
          (acc: OptionItem[], { keywords = [] }) => {
    ...
    },
    {
      ...
      type: filterBuilderTypes.EXACT,
      optionsSelector: function (items: Game[]): OptionItem[] {
        const certificationList = items.reduce((acc: OptionItem[], game) => {
    ...
    },
    {
      ...
      type: filterBuilderTypes.EXACT,
      optionsSelector: function (items: Game[]): OptionItem[] {
        const gameTypeList = items.reduce((acc: OptionItem[], game) => {
...
export const reducers = createHandleActions(
  {
    ...
    [SET_GAME_VIEW]: function (
      state: typeof defaultState,
      { payload }: { payload: { view: string } }
    ) {
    ...
    [SET_GAME_POSTER_OPTION]: function (
      state: typeof defaultState,
    ...
    [SET_GAME_OVERVIEW_OPTION]: function (
      state: typeof defaultState,
      { payload }: { payload: Record<string, unknown> }
    ) {
