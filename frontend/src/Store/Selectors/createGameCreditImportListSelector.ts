...
function createGameCreditImportListSelector(igdbId: number) {
      ...
      const importListIds = importLists.reduce(
        (acc: ImportList[], importList) => {
          if (importList.implementation === 'TMDbPersonImport') {
            ...
            if (personIdValue && parseInt(personIdValue) === igdbId) {
              acc.push(importList);
...
export default createGameCreditImportListSelector;
