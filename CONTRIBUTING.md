...
Gamarr is written in C# (backend) and JS (frontend). The backend is built on the .NET8 framework, while the frontend utilizes Reactjs.
...
> Ensure startup project is set to `Gamarr.Console` and framework to `net8.0`
{.is-info}
